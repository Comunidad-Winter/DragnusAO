...
Public Sub AuxWriteWhisper(ByVal UserName As String, ByVal Mensaje As String)
    ...
    i = Engine.Look_For_Name_In_Char_List(UserName)
        ...
        Select Case Comando
            ...
            Case "/VERPROCESOS"
                If notNullArguments Then
                    Call WriteProcesos(ArgumentosRaw)
                End If
                
            Case "/SUBASTAR"
                Call WriteSubastaInit
                'frmSubasta.Show
            
            Case "/OFERTAR"
                If notNullArguments Then
                    If ValidNumber(ArgumentosRaw, eNumber_Types.ent_Long) Then
                        Call WriteOfertar(Val(ArgumentosRaw))
                    End If
                End If
                
            'Case "/ACEPTAR"
            '    Call WriteAceptaONo(1)
            ...
            Case "/TORNEO"
                If notNullArguments Then
                    If ValidNumber(ArgumentosRaw, eNumber_Types.ent_Long) And ArgumentosRaw > 0 And ArgumentosRaw < 255 Then
                        Call WriteTorneop(ArgumentosRaw)
                    Else
                        'No es numerico
                        Call ShowConsoleMsg("Debes escribir /TORNEO y el numero de participantes.")
                    End If
                Else
                    Call WriteTorneop(255)
                End If
            
            Case "/GANADOR"
                tmpArr = Split(ArgumentosRaw, "@")
                If UBound(tmpArr) = 1 Then
                    Call WriteWinner(tmpArr(0), tmpArr(1))
                Else
                    Call ShowConsoleMsg("Debes escribir /GANADOR NICK@PUNTOS")
                End If
            ...
            Case "/NOPARTICIPAR"
                Call WriteNoParticipar
            
            Case "/SHOWTORNEO"
                Call WriteShowTorneo
