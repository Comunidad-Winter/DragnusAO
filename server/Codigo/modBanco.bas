...
UserList(UserIndex).UserAccount.Boveda.Object(Slot) = Object
...
If Not UpdateAll Then
    ...
    If UserList(UserIndex).UserAccount.Boveda.Object(Slot).ObjIndex > 0 Then
        Call SendBanObj(UserIndex, Slot, UserList(UserIndex).UserAccount.Boveda.Object(Slot))
    Else
...
'Actualiza todos los slots
    For LoopC = 1 To MAX_BANCOINVENTORY_SLOTS
        ...
        'Actualiza el inventario
        If UserList(UserIndex).UserAccount.Boveda.Object(LoopC).ObjIndex > 0 Then
            Call SendBanObj(UserIndex, LoopC, UserList(UserIndex).UserAccount.Boveda.Object(LoopC))
        Else
...
On Error GoTo errhandler
    ...
    If UserList(UserIndex).UserAccount.Boveda.Object(i).amount > 0 Then
         If Cantidad > UserList(UserIndex).UserAccount.Boveda.Object(i).amount Then Cantidad = UserList(UserIndex).UserAccount.Boveda.Object(i).amount
...
If UserList(UserIndex).UserAccount.Boveda.Object(ObjIndex).amount <= 0 Then Exit Sub

obji = UserList(UserIndex).UserAccount.Boveda.Object(ObjIndex).ObjIndex
...
ObjIndex = UserList(UserIndex).UserAccount.Boveda.Object(Slot).ObjIndex
    ...
    'Quita un Obj
       ...
       UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount = UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount - Cantidad
        
        If UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount <= 0 Then
            UserList(UserIndex).UserAccount.Boveda.NroItems = UserList(UserIndex).UserAccount.Boveda.NroItems - 1
            UserList(UserIndex).UserAccount.Boveda.Object(Slot).ObjIndex = 0
            UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount = 0
        End If
...
'¿Ya tiene un objeto de este tipo?
Slot = 1
Do Until UserList(UserIndex).UserAccount.Boveda.Object(Slot).ObjIndex = obji And _
         UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount + Cantidad <= MAX_INVENTORY_OBJS
            Slot = Slot + 1
...
If Slot > MAX_BANCOINVENTORY_SLOTS Then
        Slot = 1
        Do Until UserList(UserIndex).UserAccount.Boveda.Object(Slot).ObjIndex = 0
            Slot = Slot + 1
            ...
            End If
        Loop
        If Slot <= MAX_BANCOINVENTORY_SLOTS Then UserList(UserIndex).UserAccount.Boveda.NroItems = UserList(UserIndex).UserAccount.Boveda.NroItems + 1
...
If Slot <= MAX_BANCOINVENTORY_SLOTS Then 'Slot valido
    'Mete el obj en el slot
    If UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount + Cantidad <= MAX_INVENTORY_OBJS Then
        
        'Menor que MAX_INV_OBJS
        UserList(UserIndex).UserAccount.Boveda.Object(Slot).ObjIndex = obji
        UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount = UserList(UserIndex).UserAccount.Boveda.Object(Slot).amount + Cantidad
...
Else
    Call WriteConsoleMsg(UserIndex, "El banco esta lleno.", FontTypeNames.FONTTYPE_INFO)
End If
...
Call WriteConsoleMsg(sendIndex, " Tiene " & UserList(UserIndex).UserAccount.Boveda.NroItems & " objetos.", FontTypeNames.FONTTYPE_INFO)

For j = 1 To MAX_BANCOINVENTORY_SLOTS
    If UserList(UserIndex).UserAccount.Boveda.Object(j).ObjIndex > 0 Then
        Call WriteConsoleMsg(sendIndex, " Objeto " & j & " " & ObjData(UserList(UserIndex).UserAccount.Boveda.Object(j).ObjIndex).name & " Cantidad:" & UserList(UserIndex).UserAccount.Boveda.Object(j).amount, FontTypeNames.FONTTYPE_INFO)
    End If
Next
...
If FileExist(CharFile, vbNormal) Then
    ...
    Call WriteConsoleMsg(sendIndex, " Tiene " & GetVar(CharFile, "UserAccount.Bovedaory", "CantidadItems") & " objetos.", FontTypeNames.FONTTYPE_INFO)
    For j = 1 To MAX_BANCOINVENTORY_SLOTS
        Tmp = GetVar(CharFile, "UserAccount.Bovedaory", "Obj" & j)
