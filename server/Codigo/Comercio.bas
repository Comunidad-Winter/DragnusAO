...
On Error GoTo errorh
    ...
    If UserList(UserIndex).Invent.Object(Slot).amount + Cantidad <= MAX_INVENTORY_OBJS Then
        ...
        If ObjData(obji).OBJType = eOBJType.otLlaves Then
            ...
            Call WriteVar(DatPath & "NPCs.dat", "NPC" & Npclist(NpcIndex).Numero, "obj" & ObjIndex, obji & "-0")
        End If
...
Sub EnviarNpcInv(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    For i = 1 To MAX_INVENTORY_SLOTS
        If Npclist(NpcIndex).Invent.Object(i).ObjIndex > 0 Then
            ...
            Call WriteChangeNPCInventorySlot(UserIndex, i, thisObj, val)
        Else
             Dim DummyObj As Obj
             Call WriteChangeNPCInventorySlot(UserIndex, i, DummyObj, val)
        End If
    Next i
End Sub
