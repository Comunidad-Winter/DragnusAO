...
'            'Call apicloseConnection(s)
'            call WSApicloseConnection(s)
'            Exit Function
'        End If
'
...
''        Loop
    ...
    Case FD_CLOSE
        ...
        N = BuscaSlotSock(S)
        If S <> SockListen Then Call apicloseConnection(S)
...
'Modificado por Maraxus
    ...
    If Not SecurityIp.IpSecurityAceptarNuevaConexion(sa.sin_addr) Then
        Call WSApicloseConnection(NuevoSock)
        Exit Sub
    End If
    ...
    If NewIndex <= MaxUsers Then
        ...
        For i = 1 To BanIps.Count
            If BanIps.Item(i) = UserList(NewIndex).ip Then
                'Call apicloseConnection(NuevoSock)
...
Public Sub WSApicloseConnection(ByVal Socket As Long)
#If UsarQueSocket = 1 Then
